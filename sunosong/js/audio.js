...
var musicListTitle = document.getElementById('music-list-title');
// 左侧关闭面板
...
musicListTitle.addEventListener('mousedown', function (event) {
    musicList.classList.remove("list-card-show");
    musicList.classList.add("list-card-hide");
    closeList.style.display = "none";
});
...
function initMusic() {
    ...
    audio.src = musicData[musicId]['mp3'];;
    audio.load();
    recordImg.classList.remove('rotate-play');
    audio.ondurationchange = function () {
        musicTitle.innerText = musicData[musicId]['name2'];
        // author.innerText = musicData[musicId][1];
        author.innerText = '木子才';
        recordImg.style.backgroundImage = "url('./../song/img/record"+Math.floor(Math.random() * 4).toString()+".jpg')";
        body.style.backgroundImage = "url('./../song/img/bg"+Math.floor(Math.random() * 4).toString()+".png')";
...
mode.addEventListener('click', function (event) {
    ...
    mode.style.backgroundImage = "url('./../song/img/mode" + modeId.toString() + ".png')";
});
...
function setNoVolumn() {
    if (volumeTogger.value == 0) {
        ...
        volume.style.backgroundImage = "url('./../song/img/音量.png')";
    }
    else {
        lastVolumn = volumeTogger.value;
        volumeTogger.value = 0;
        volume.style.backgroundImage = "url('./../song/img/静音.png')";
    }
}
